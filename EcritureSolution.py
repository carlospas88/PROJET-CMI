...
def ecritureSolution(TabSolution,nbZone,nbSecteur,adressesolution):
    fichier = open(adressesolution,'w')
    solution=csv.writer(fichier,delimiter=";")
    STRNB = [str(nbSecteur)]
    solution.writerow(STRNB)
    ...
    #si on n'est pas dans le cas de l'hypothèse 1, alors on est dans le cas des hypothèses 2 et 3 avec des zones
        ...
        #print(TabSolution)
        for i in range(nbZone):
                solution.writerow(TabSolution[i])